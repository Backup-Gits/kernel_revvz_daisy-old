...
static void fg_handle_battery_insertion(struct fg_chip *chip)
{
	...
	mod_delayed_work(system_freezable_power_efficient_wq, &chip->batt_profile_init, 0);
	cancel_delayed_work(&chip->update_sram_data);
	mod_delayed_work(system_freezable_power_efficient_wq, &chip->update_sram_data, msecs_to_jiffies(0));
}
...
static void status_change_work(struct work_struct *work)
{
	...
	if (chip->prev_status != chip->status && chip->last_sram_update_time) {
		...
		if (chip->last_temp_update_time && chip->soc_slope_limiter_en) {
			...
			mod_delayed_work(system_freezable_power_efficient_wq, &chip->update_temp_work,
				msecs_to_jiffies(0));
		}
		...
		if (chip->last_sram_update_time + 5 < current_time) {
			cancel_delayed_work(&chip->update_sram_data);
			mod_delayed_work(system_freezable_power_efficient_wq, &chip->update_sram_data,
				msecs_to_jiffies(0));
		}
		if (chip->cyc_ctr.en)
...
static irqreturn_t fg_vbatt_low_handler(int irq, void *_chip)
{
	...
	if (chip->use_vbat_low_empty_soc) {
		...
		if (vbatt_low_sts) {
			...
			disable_irq_nosync(chip->batt_irq[VBATT_LOW].irq);
			chip->vbat_low_irq_enabled = false;
			mod_delayed_work(system_freezable_power_efficient_wq, &chip->check_empty_work,
...
out:
	...
	mod_delayed_work(system_freezable_power_efficient_wq, &chip->check_sanity_work,
